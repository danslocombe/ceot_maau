#!/usr/bin/env python3
"""
Parameter sweep for JYUTPING_COMPLETION_PENALTY_K and JYUTPING_NON_PERFECT_MATCH.

For each (completion_k, non_perfect) pair:
...
Usage:
  python eval/experiments/partial_prefix/sweep.py
"""
...
def patch_constants(content, completion_k, non_perfect):
...
def main():
    ...
    completion_k_values = [2_000, 4_000, 6_000, 8_000]
    non_perfect_values = [3_000, 6_000, 9_000]
    ...
    safe_print(f"Partial prefix sweep: {len(pairs)} combinations")
    ...
    try:
        for i, (ck_val, np_val) in enumerate(pairs):
            safe_print(f"\n{'='*60}")
            safe_print(f"[{i+1}/{len(pairs)}] COMPLETION_K={ck_val}, NON_PERFECT={np_val}")
            safe_print(f"{'='*60}")

            # Patch
            patched = patch_constants(original_source, ck_val, np_val)
            ...
            entry = {
                ...
                "pp_p3": pp.get("p@3", 0),
                "pp_mrr": pp.get("mrr", 0),
                "pp_not_found": pp.get("not_found", 0),
            ...
            safe_print(
                f"  partial_prefix: p@1={_fmt_pct(pp.get('p@1',0))} p@3={_fmt_pct(pp.get('p@3',0))} "
                f"MRR={_fmt_mrr(pp.get('mrr',0))}"
            )
            safe_print(
                f"  exact_vs_prefix: p@1={_fmt_pct(evp.get('p@1',0))} MRR={_fmt_mrr(evp.get('mrr',0))}"
            )
            safe_print(
                f"  single_tone: p@1={_fmt_pct(st.get('p@1',0))} | multi_syl: p@1={_fmt_pct(ms.get('p@1',0))} | shorter_entry: p@1={_fmt_pct(se.get('p@1',0))}"
            )
    ...
    safe_print(f"{'='*110}")
    safe_print(
        f"{'CK':>6} {'NP':>6} | {'All p@1':>7} {'All p@3':>7} {'All MRR':>7} {'Miss':>4} | "
        f"{'PP p@1':>7} {'PP p@3':>7} {'PP MRR':>7} | "
        f"{'EVP p@1':>7} {'EVP MRR':>7} | "
        f"{'ST p@1':>7} {'MS p@1':>7} {'SE p@1':>7}"
    )
    safe_print(f"{'-'*6} {'-'*6}-+-{'-'*7}-{'-'*7}-{'-'*7}-{'-'*4}-+-{'-'*7}-{'-'*7}-{'-'*7}-+-{'-'*7}-{'-'*7}-+-{'-'*7}-{'-'*7}-{'-'*7}")

    for r in results_log:
        if "error" in r:
            safe_print(f"{r['completion_k']:>6} {r['non_perfect']:>6} | {'ERROR':>7}")
            continue
        safe_print(
            f"{r['completion_k']:>6} {r['non_perfect']:>6} | "
            f"{_fmt_pct(r['overall_p1']):>7} {_fmt_pct(r['overall_p3']):>7} {_fmt_mrr(r['overall_mrr']):>7} {r['overall_not_found']:>4} | "
            f"{_fmt_pct(r['pp_p1']):>7} {_fmt_pct(r['pp_p3']):>7} {_fmt_mrr(r['pp_mrr']):>7} | "
            f"{_fmt_pct(r['evp_p1']):>7} {_fmt_mrr(r['evp_mrr']):>7} | "
            f"{_fmt_pct(r['st_p1']):>7} {_fmt_pct(r['ms_p1']):>7} {_fmt_pct(r['se_p1']):>7}"
        )

    valid = [r for r in results_log if "error" not in r]
    if valid:
        best_overall = max(valid, key=lambda r: r["overall_mrr"])
        best_pp = max(valid, key=lambda r: r["pp_mrr"])
        safe_print(f"\nBest overall MRR: CK={best_overall['completion_k']} NP={best_overall['non_perfect']} "
                    f"-> MRR={_fmt_mrr(best_overall['overall_mrr'])}")
        safe_print(f"Best PP MRR:      CK={best_pp['completion_k']} NP={best_pp['non_perfect']} "
                    f"-> MRR={_fmt_mrr(best_pp['pp_mrr'])}")
